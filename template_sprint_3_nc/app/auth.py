...
def activate():
    try:
        ...
        if request.method == 'GET':
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM activatelink WHERE challenge=? and state =? and CURRENT_TIMESTAMP BETWEEN create and validuntil', (number, utils.U_UNCONFIRMED)
            ).fetchone()

            if attempt is not None:
                db.execute(
                    'UPDATE activationlink state=? id?', (utils.U_CONFIRMED, attempt['id'])
                )
                db.execute(
...
def register():
    try:
        ...
        if request.method == 'GET':
            ...
            db.execute(
                'INSERT INTO activationlink (username, challenge, state,password, salt, email) VALUES (?, ?, ?, ?, ?, ?)',
...
def confirm():
    try:
        ...
        if request.method == 'POST':
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM activatelink WHERE challenge=? and state =? and CURRENT_TIMESTAMP BETWEEN create and validuntil', (authid, utils.F_ACTIVE)
            ).fetchone()
...
def change():
    try:
        ...
        if request.method == 'GET':
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM forgotlink WHERE challenge=? and state =? and CURRENT_TIMESTAMP BETWEEN create and validuntil', (number, utils.F_ACTIVE)
            ).fetchone()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                db.execute(
                    'INSERT INTO forgotlink (userid , chellenge, state) VALUE (?, ? , ?)',
